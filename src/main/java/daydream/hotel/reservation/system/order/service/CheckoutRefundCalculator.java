...
public final class CheckoutRefundCalculator {
    ...
    public static CheckoutRefundResult calculate(
        ...
        if (paidAmount == null || paidAmount.compareTo(BigDecimal.ZERO) <= 0) {
            return new CheckoutRefundResult(BigDecimal.ZERO, BigDecimal.ZERO, 0, null, "未支付，无法退款");
        }
        if (orderNights <= 0) {
            return new CheckoutRefundResult(BigDecimal.ZERO, paidAmount, 0, null, "订单晚数异常，无法退款");
        }
        ...
        if (!refundFrom.isBefore(checkOutDate)) {
            return new CheckoutRefundResult(
                    BigDecimal.ZERO, paidAmount, 0, refundFrom, "当前无可退晚数（当日18:00后申请不退当天房费）");
        }
        ...
        String timeHint =
                applyTime.toLocalTime().isBefore(CHECKOUT_CUTOFF)
                        ? "18:00前申请，含当天房费"
                        : "18:00后申请，不含当天房费";
        String policy = String.format("退还剩余%d晚房费¥%s（%s）", refundableNights, refundAmount, timeHint);
