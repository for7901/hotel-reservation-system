...
import { ORDER_STATUS, ORDER_STATUS_FILTERS, type Order } from '@/types/order'
...
async function handleApproveCheckout() {
  ...
  try {
    await ElMessageBox.confirm(
      `确认退房并退款 ¥${currentOrder.value.refundAmount ?? '-'}？订单将变为已退款。`,
      '确认退款',
      { type: 'warning' },
    )
    reviewing.value = true
    currentOrder.value = await approveCheckout(currentOrder.value.id)
    ElMessage.success('已确认退房，订单已退款')
    await loadData()
  } catch (e) {
    if (e !== 'cancel') {
      ElMessage.error(e instanceof Error ? e.message : '操作失败')
  ...
  <el-card>
    ...
    <el-table v-loading="loading" :data="orders" stripe>
      ...
      <el-table-column label="间数" width="80">
        <template #default="{ row }">{{ row.roomCount || row.guestCount || 1 }} 间</template>
      </el-table-column>
    ...
    <div v-loading="detailLoading">
      <template v-if="currentOrder">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="预订间数">{{ currentOrder.roomCount || currentOrder.guestCount || 1 }} 间</el-descriptions-item>
        ...
        <div class="guest-section">
          <div class="guest-title">各房间入住人</div>
          <el-table :data="currentOrder.guests || []" size="small" stripe>
            <el-table-column type="index" label="房间" width="70" />
            <el-table-column prop="name" label="住客姓名" min-width="140" />
